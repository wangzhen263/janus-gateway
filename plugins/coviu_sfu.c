/*! \file   coviu_sfu.c
 * \author Jeff Wang <jeff.wang@coviu.com>
 * \copyright GNU General Public License v3
 * \brief  Modified from coviu_sfu plugin
...
/* Plugin information */
#define COVIU_SFU_VERSION			1
#define COVIU_SFU_VERSION_STRING	"0.0.1"
#define COVIU_SFU_DESCRIPTION		"This is a plugin implementing a videoconferencing SFU (Selective Forwarding Unit) for COVIU, that is an audio/video/data router."
#define COVIU_SFU_NAME			"COVIU SFU plugin"
#define COVIU_SFU_AUTHOR			"Jeff Wang"
#define COVIU_SFU_PACKAGE			"janus.plugin.coviusfu"

/* Plugin methods */
janus_plugin *create(void);
int coviu_sfu_init(janus_callbacks *callback, const char *config_path);
void coviu_sfu_destroy(void);
int coviu_sfu_get_api_compatibility(void);
int coviu_sfu_get_version(void);
const char *coviu_sfu_get_version_string(void);
const char *coviu_sfu_get_description(void);
const char *coviu_sfu_get_name(void);
const char *coviu_sfu_get_author(void);
const char *coviu_sfu_get_package(void);
void coviu_sfu_create_session(janus_plugin_session *handle, int *error);
struct janus_plugin_result *coviu_sfu_handle_message(janus_plugin_session *handle, char *transaction, char *message, char *sdp_type, char *sdp);
void coviu_sfu_setup_media(janus_plugin_session *handle);
void coviu_sfu_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len);
void coviu_sfu_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len);
void coviu_sfu_incoming_data(janus_plugin_session *handle, char *buf, int len);
void coviu_sfu_slow_link(janus_plugin_session *handle, int uplink, int video);
void coviu_sfu_hangup_media(janus_plugin_session *handle);
void coviu_sfu_destroy_session(janus_plugin_session *handle, int *error);
char *coviu_sfu_query_session(janus_plugin_session *handle);

/* Plugin setup */
static janus_plugin coviu_sfu_plugin =
	JANUS_PLUGIN_INIT (
		.init = coviu_sfu_init,
		.destroy = coviu_sfu_destroy,

		.get_api_compatibility = coviu_sfu_get_api_compatibility,
		.get_version = coviu_sfu_get_version,
		.get_version_string = coviu_sfu_get_version_string,
		.get_description = coviu_sfu_get_description,
		.get_name = coviu_sfu_get_name,
		.get_author = coviu_sfu_get_author,
		.get_package = coviu_sfu_get_package,
		
		.create_session = coviu_sfu_create_session,
		.handle_message = coviu_sfu_handle_message,
		.setup_media = coviu_sfu_setup_media,
		.incoming_rtp = coviu_sfu_incoming_rtp,
		.incoming_rtcp = coviu_sfu_incoming_rtcp,
		.incoming_data = coviu_sfu_incoming_data,
		.slow_link = coviu_sfu_slow_link,
		.hangup_media = coviu_sfu_hangup_media,
		.destroy_session = coviu_sfu_destroy_session,
		.query_session = coviu_sfu_query_session,
	);

/* Plugin creator */
janus_plugin *create(void) {
	JANUS_LOG(LOG_VERB, "%s created!\n", COVIU_SFU_NAME);
	return &coviu_sfu_plugin;
}
...
static void *coviu_sfu_handler(void *data);
static void coviu_sfu_relay_rtp_packet(gpointer data, gpointer user_data);
static void coviu_sfu_relay_data_packet(gpointer data, gpointer user_data);

typedef enum coviu_sfu_p_type {
	coviu_sfu_p_type_none = 0,
	coviu_sfu_p_type_subscriber,			/* Generic listener/subscriber */
	coviu_sfu_p_type_subscriber_muxed,	/* Multiplexed listener/subscriber */
	coviu_sfu_p_type_publisher,			/* Participant/publisher */
} coviu_sfu_p_type;

typedef struct coviu_sfu_message {
...
static coviu_sfu_message exit_message;

static void coviu_sfu_message_free(coviu_sfu_message *msg) {
...
typedef enum coviu_sfu_audiocodec {
	COVIU_SFU_OPUS,		/* Publishers will have to use OPUS 	*/
	COVIU_SFU_ISAC_32K,	/* Publishers will have to use ISAC 32K */
	COVIU_SFU_ISAC_16K,	/* Publishers will have to use ISAC 16K */
	COVIU_SFU_PCMU,		/* Publishers will have to use PCMU 8K 	*/
	COVIU_SFU_PCMA		/* Publishers will have to use PCMA 8K 	*/
} coviu_sfu_audiocodec;
static const char *coviu_sfu_audiocodec_name(coviu_sfu_audiocodec acodec) {
	switch(acodec) {
		case COVIU_SFU_OPUS:
			return "opus";
		case COVIU_SFU_ISAC_32K:
			return "isac32";
		case COVIU_SFU_ISAC_16K:
			return "isac16";
		case COVIU_SFU_PCMU:
			return "pcmu";
		case COVIU_SFU_PCMA:
...
typedef enum coviu_sfu_videocodec {
	COVIU_SFU_VP8,	/* Publishers will have to use VP8 */
	COVIU_SFU_VP9,	/* Publishers will have to use VP9 */
	COVIU_SFU_H264	/* Publishers will have to use H264 */
} coviu_sfu_videocodec;
static const char *coviu_sfu_videocodec_name(coviu_sfu_videocodec vcodec) {
	switch(vcodec) {
		case COVIU_SFU_VP8:
			return "vp8";
		case COVIU_SFU_VP9:
...
typedef struct coviu_sfu {
	...
	coviu_sfu_audiocodec acodec;	/* Audio codec to force on publishers*/
	coviu_sfu_videocodec vcodec;	/* Video codec to force on publishers*/
...
static void coviu_sfu_free(coviu_sfu *room);

typedef struct coviu_sfu_session {
	janus_plugin_session *handle;
	coviu_sfu_p_type participant_type;
...
typedef struct coviu_sfu_participant {
	coviu_sfu_session *session;
	coviu_sfu *room;	/* Room */
...
} coviu_sfu_participant;
static void coviu_sfu_participant_free(coviu_sfu_participant *p);
static void janus_rtp_forwarder_free_helper(gpointer data);
static guint32 janus_rtp_forwarder_add_helper(coviu_sfu_participant *p, const gchar* host, int port, int is_video);
typedef struct coviu_sfu_listener_context {
...
} coviu_sfu_listener_context;

typedef struct coviu_sfu_listener {
	coviu_sfu_session *session;
	coviu_sfu *room;	/* Room */
	coviu_sfu_participant *feed;	/* Participant this listener is subscribed to */
	coviu_sfu_listener_context context;	/* Needed in case there are publisher switches on this listener */
	gboolean audio, video, data;		/* Whether audio, video and/or data must be sent to this publisher */
	struct coviu_sfu_listener_muxed *parent;	/* Overall subscriber, if this is a sub-listener in a Multiplexed one */
	gboolean paused;
} coviu_sfu_listener;
static void coviu_sfu_listener_free(coviu_sfu_listener *l);

typedef struct coviu_sfu_listener_muxed {
	coviu_sfu_session *session;
	coviu_sfu *room;	/* Room */
	GSList *listeners;	/* List of listeners (as a Multiplexed listener can be subscribed to more publishers at the same time) */
	janus_mutex listeners_mutex;
} coviu_sfu_listener_muxed;
static void coviu_sfu_muxed_listener_free(coviu_sfu_listener_muxed *l);

typedef struct coviu_sfu_rtp_relay_packet {
	rtp_header *data;
	gint length;
	gint is_video;
	uint32_t timestamp;
	uint16_t seq_number;
} coviu_sfu_rtp_relay_packet;
...
/* Error codes */
#define COVIU_SFU_ERROR_UNKNOWN_ERROR		499
#define COVIU_SFU_ERROR_NO_MESSAGE			421
#define COVIU_SFU_ERROR_INVALID_JSON		422
#define COVIU_SFU_ERROR_INVALID_REQUEST		423
#define COVIU_SFU_ERROR_JOIN_FIRST			424
#define COVIU_SFU_ERROR_ALREADY_JOINED		425
#define COVIU_SFU_ERROR_NO_SUCH_ROOM		426
#define COVIU_SFU_ERROR_ROOM_EXISTS			427
#define COVIU_SFU_ERROR_NO_SUCH_FEED		428
#define COVIU_SFU_ERROR_MISSING_ELEMENT		429
#define COVIU_SFU_ERROR_INVALID_ELEMENT		430
#define COVIU_SFU_ERROR_INVALID_SDP_TYPE	431
#define COVIU_SFU_ERROR_PUBLISHERS_FULL		432
#define COVIU_SFU_ERROR_UNAUTHORIZED		433
#define COVIU_SFU_ERROR_ALREADY_PUBLISHED	434
#define COVIU_SFU_ERROR_NOT_PUBLISHED		435
#define COVIU_SFU_ERROR_ID_EXISTS			436
#define COVIU_SFU_ERROR_INVALID_SDP			437


/* Multiplexing helpers */
int coviu_sfu_muxed_subscribe(coviu_sfu_listener_muxed *muxed_listener, GList *feeds, char *transaction);
int coviu_sfu_muxed_unsubscribe(coviu_sfu_listener_muxed *muxed_listener, GList *feeds, char *transaction);
int coviu_sfu_muxed_offer(coviu_sfu_listener_muxed *muxed_listener, char *transaction, char *event_text);
...
static void session_free(gpointer data) {
	if(data) {
		coviu_sfu_session* session = (coviu_sfu_session*)data;
		switch(session->participant_type) {
		case coviu_sfu_p_type_publisher: 
			coviu_sfu_participant_free(session->participant);
			break;   
		case coviu_sfu_p_type_subscriber:
			coviu_sfu_listener_free(session->participant);
			break;
		case coviu_sfu_p_type_subscriber_muxed:
			coviu_sfu_muxed_listener_free(session->participant);
			break;
		default:
			break;
		}
...
void *coviu_sfu_watchdog(void *data);
void *coviu_sfu_watchdog(void *data) {
	JANUS_LOG(LOG_INFO, "coviu sfu watchdog started\n");
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(old_sessions != NULL) {
			...
			while(sl) {
				coviu_sfu_session *session = (coviu_sfu_session *)sl->data;
				...
				if(now-session->destroyed >= 5*G_USEC_PER_SEC) {
					...
					JANUS_LOG(LOG_VERB, "Freeing old SFU session\n");
					GList *rm = sl->next;
		...
		if(old_rooms != NULL) {
			...
			while(rl) {
				coviu_sfu* room = (coviu_sfu*)rl->data;
				if(!initialized || stopping){
					break;
				}
				if(!room) {
	...
	JANUS_LOG(LOG_INFO, "SFU watchdog stopped\n");
	return NULL;
}
...
int coviu_sfu_init(janus_callbacks *callback, const char *config_path) {
	...
	/* Read configuration */
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.cfg", config_path, COVIU_SFU_PACKAGE);
	...
	rooms = g_hash_table_new_full(g_int64_hash, g_int64_equal,
		(GDestroyNotify)g_free, (GDestroyNotify) coviu_sfu_free);
	janus_mutex_init(&rooms_mutex);
	sessions = g_hash_table_new(NULL, NULL);
	janus_mutex_init(&sessions_mutex);

	messages = g_async_queue_new_full((GDestroyNotify) coviu_sfu_message_free);
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			...
			coviu_sfu *videoroom = g_malloc0(sizeof(coviu_sfu));
			if(videoroom == NULL) {
			...
			JANUS_LOG(LOG_VERB, "Created videoroom: %"SCNu64" (%s, %s, %s/%s codecs, secret: %s, pin: %s)\n",
				...
				coviu_sfu_audiocodec_name(videoroom->acodec),
				coviu_sfu_videocodec_name(videoroom->vcodec),
	...
	while (g_hash_table_iter_next(&iter, NULL, &value)) {
		...
		JANUS_LOG(LOG_VERB, "  ::: [%"SCNu64"][%s] %"SCNu64", max %d publishers, FIR frequency of %d seconds, %s audio codec, %s video codec\n",
			...
			coviu_sfu_audiocodec_name(vr->acodec), coviu_sfu_videocodec_name(vr->vcodec));
	}
	janus_mutex_unlock(&rooms_mutex);
	...
	watchdog = g_thread_try_new("videoroom watchdog", &coviu_sfu_watchdog, NULL, &error);
	if(error != NULL) {
	...
	handler_thread = g_thread_try_new("videoroom handler", coviu_sfu_handler, NULL, &error);
	if(error != NULL) {
		g_atomic_int_set(&initialized, 0);
...
int coviu_sfu_get_version(void) {
	return COVIU_SFU_VERSION;
}

const char *coviu_sfu_get_version_string(void) {
	return COVIU_SFU_VERSION_STRING;
}

const char *coviu_sfu_get_description(void) {
	return COVIU_SFU_DESCRIPTION;
}

const char *coviu_sfu_get_name(void) {
	return COVIU_SFU_NAME;
}

const char *coviu_sfu_get_author(void) {
	return COVIU_SFU_AUTHOR;
}

const char *coviu_sfu_get_package(void) {
	return COVIU_SFU_PACKAGE;
}
...
void coviu_sfu_create_session(janus_plugin_session *handle, int *error) {
	...
	coviu_sfu_session *session = (coviu_sfu_session *)g_malloc0(sizeof(coviu_sfu_session));
	if(session == NULL) {
		JANUS_LOG(LOG_FATAL, "Memory error!\n");
		*error = -2;
		return;
	...
	session->participant_type = coviu_sfu_p_type_none;
...
static void coviu_sfu_notify_participants(coviu_sfu_participant *participant, char *msg) {
	...
	while (!participant->room->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
		coviu_sfu_participant *p = value;
		if(p && p->session && p != participant) {
			JANUS_LOG(LOG_VERB, "Notifying participant %"SCNu64" (%s)\n", p->user_id, p->display ? p->display : "??");
			int ret = gateway->push_event(p->session->handle, &coviu_sfu_plugin, NULL, msg, NULL, NULL);
			JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
		}
	}
}

static void coviu_sfu_leave_or_unpublish(coviu_sfu_participant *participant, gboolean is_leaving) {
	...
	if(participant->room) {
		if(!participant->room->destroyed) {
			...
			coviu_sfu_notify_participants(participant, leaving_text);
			if(is_leaving) {
...
void coviu_sfu_destroy_session(janus_plugin_session *handle, int *error) {
	...
	coviu_sfu_session *session = (coviu_sfu_session *)handle->plugin_handle; 
	if(!session) {
	...
	if(!session->destroyed) {
		...
		coviu_sfu_hangup_media(handle);
		session->destroyed = janus_get_monotonic_time();
		old_sessions = g_list_append(old_sessions, session);
		if(session->participant_type == coviu_sfu_p_type_publisher) {
			/* Get rid of publisher */
			coviu_sfu_participant *participant = (coviu_sfu_participant *)session->participant;
			...
			coviu_sfu_leave_or_unpublish(participant, TRUE);
		} else if(session->participant_type == coviu_sfu_p_type_subscriber) {
			/* Detaching this listener from its publisher is already done by hangup_media */
		} else if(session->participant_type == coviu_sfu_p_type_subscriber_muxed) {
...
char *coviu_sfu_query_session(janus_plugin_session *handle) {
	...
	if(session->participant) {
		if(session->participant_type == coviu_sfu_p_type_none) {
			json_object_set_new(info, "type", json_string("none"));
		} else if(session->participant_type == coviu_sfu_p_type_publisher) {
			json_object_set_new(info, "type", json_string("publisher"));
			coviu_sfu_participant *participant = (coviu_sfu_participant *)session->participant;
			if(participant) {
				coviu_sfu *room = participant->room; 
			...
			coviu_sfu_listener *participant = (coviu_sfu_listener *)session->participant;
			if(participant) {
				coviu_sfu_participant *feed = (coviu_sfu_participant *)participant->feed;
				if(feed) {
					coviu_sfu *room = feed->room;
					json_object_set_new(info, "room", room ? json_integer(room->room_id) : NULL);
...
static int coviu_sfu_access_room(json_t *root, gboolean check_secret, gboolean check_pin, coviu_sfu **videoroom, char *error_cause, int error_cause_size) {
	...
	*videoroom = g_hash_table_lookup(rooms, &room_id);
	if(*videoroom == NULL) {
		...
		error_code = COVIU_SFU_ERROR_NO_SUCH_ROOM;
		if(error_cause)
	...
	if(check_secret) {
		...
		JANUS_CHECK_SECRET((*videoroom)->room_secret, root, "secret", error_code, error_cause2,
			COVIU_SFU_ERROR_MISSING_ELEMENT, COVIU_SFU_ERROR_INVALID_ELEMENT, COVIU_SFU_ERROR_UNAUTHORIZED);
		if(error_code != 0) {
...
struct janus_plugin_result *coviu_sfu_handle_message(janus_plugin_session *handle, char *transaction, char *message, char *sdp_type, char *sdp) {
	...
	if(message == NULL) {
		...
		error_code = COVIU_SFU_ERROR_NO_MESSAGE;
	...
	if(!session) {
		JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
		error_code = COVIU_SFU_ERROR_UNKNOWN_ERROR;
	...
	if(!root) {
		...
		error_code = COVIU_SFU_ERROR_INVALID_JSON;
	...
	JANUS_VALIDATE_JSON_OBJECT(root, request_parameters,
		...
		COVIU_SFU_ERROR_MISSING_ELEMENT, COVIU_SFU_ERROR_INVALID_ELEMENT);
	if(error_code != 0)
		goto error;
	...
	if(!strcasecmp(request_text, "create")) {
		...
		if(audiocodec) {
			...
			if(!strcasecmp(audiocodec_value, "opus") && !strcasecmp(audiocodec_value, "isac32") && !strcasecmp(audiocodec_value, "isac16") && !strcasecmp(audiocodec_value, "pcmu") && !strcasecmp(audiocodec_value, "pcma")) {
				...
				error_code = COVIU_SFU_ERROR_INVALID_ELEMENT;
		...
		if(room_id > 0) {
			...
			if(g_hash_table_lookup(rooms, &room_id) != NULL) {
				...
				error_code = COVIU_SFU_ERROR_ROOM_EXISTS;
		...
		if(save) {
			...
			janus_config_add_item(config, cat, "audiocodec", coviu_sfu_audiocodec_name(videoroom->acodec));
			janus_config_add_item(config, cat, "videocodec", coviu_sfu_videocodec_name(videoroom->vcodec));
			...
			janus_config_save(config, config_folder, COVIU_SFU_PACKAGE);
		...
		guint64 room_id = json_integer_value(room);
		janus_mutex_lock(&rooms_mutex);
		coviu_sfu *videoroom = NULL;
		error_code = coviu_sfu_access_room(root, TRUE, FALSE, &videoroom, error_cause, sizeof(error_cause));
		if(error_code != 0) {
			janus_mutex_unlock(&rooms_mutex);
			goto error;
		}
		...
		while (g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			if(p && p->session) {
				...
				int ret = gateway->push_event(p->session->handle, &coviu_sfu_plugin, NULL, destroyed_text, NULL, NULL);
				JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
		...
		while(g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			if(!room->destroyed) {
				...
				json_object_set_new(rl, "audiocodec", json_string(coviu_sfu_audiocodec_name(room->acodec)));
				json_object_set_new(rl, "videocodec", json_string(coviu_sfu_videocodec_name(room->vcodec)));
		...
		if(error_code != 0)
			goto error;
		janus_mutex_lock(&videoroom->participants_mutex);
		coviu_sfu_participant* publisher = g_hash_table_lookup(videoroom->participants, &publisher_id);
		if(publisher == NULL) {
			janus_mutex_unlock(&videoroom->participants_mutex);
			JANUS_LOG(LOG_ERR, "No such publisher (%"SCNu64")\n", publisher_id);
			error_code = COVIU_SFU_ERROR_NO_SUCH_FEED;
		...
		janus_mutex_lock(&rooms_mutex);
		coviu_sfu *videoroom = NULL;
		error_code = coviu_sfu_access_room(root, FALSE, FALSE, &videoroom, error_cause, sizeof(error_cause));
		janus_mutex_unlock(&rooms_mutex);
		if(error_code != 0)
			goto error;
		...
		guint64 room_id = json_integer_value(room);
		janus_mutex_lock(&rooms_mutex);
		coviu_sfu *videoroom = g_hash_table_lookup(rooms, &room_id);
		if(videoroom == NULL) {
			JANUS_LOG(LOG_ERR, "No such room (%"SCNu64")\n", room_id);
			error_code = COVIU_SFU_ERROR_NO_SUCH_ROOM;
		...
		coviu_sfu_message *msg = g_malloc0(sizeof(coviu_sfu_message));
		if(msg == NULL) {
			JANUS_LOG(LOG_FATAL, "Memory error!\n");
			error_code = COVIU_SFU_ERROR_UNKNOWN_ERROR;
			g_snprintf(error_cause, 512, "Memory error");
		...
		error_code = COVIU_SFU_ERROR_INVALID_REQUEST;
...
void coviu_sfu_setup_media(janus_plugin_session *handle) {
	...
	if(session->participant) {
		...
		if(session->participant_type == coviu_sfu_p_type_publisher) {
			...
			GHashTableIter iter;
			gpointer value;
			coviu_sfu *videoroom = participant->room;
			janus_mutex_lock(&videoroom->participants_mutex);
			g_hash_table_iter_init(&iter, videoroom->participants);
			while (!videoroom->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
				coviu_sfu_participant *p = value;
				...
				JANUS_LOG(LOG_VERB, "Notifying participant %"SCNu64" (%s)\n", p->user_id, p->display ? p->display : "??");
				int ret = gateway->push_event(p->session->handle, &coviu_sfu_plugin, NULL, pub_text, NULL, NULL);
				JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
			}
			g_free(pub_text);
			janus_mutex_unlock(&videoroom->participants_mutex);
		} else if(session->participant_type == coviu_sfu_p_type_subscriber) {
			coviu_sfu_listener *l = (coviu_sfu_listener *)session->participant;
			if(l && l->feed) {
				coviu_sfu_participant *p = l->feed;
				if(p && p->session) {
					/* Send a FIR */
					char buf[20];
					memset(buf, 0, 20);
			...
			coviu_sfu_listener_muxed *listener = (coviu_sfu_listener_muxed *)session->participant;
			if(listener == NULL)
				return;
			GSList *ps = listener->listeners;
			while(ps) {
				coviu_sfu_listener *l = (coviu_sfu_listener *)ps->data;
				if(l && l->feed) {
					coviu_sfu_participant *p = l->feed;
					if(p && p->session) {
						/* Send a FIR */
...
void coviu_sfu_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len) {
	if(handle == NULL || handle->stopped || g_atomic_int_get(&stopping) || !g_atomic_int_get(&initialized) || !gateway)
		return;
	coviu_sfu_session *session = (coviu_sfu_session *)handle->plugin_handle;
	if(!session || session->destroyed || !session->participant || session->participant_type != coviu_sfu_p_type_publisher)
		return;
	coviu_sfu_participant *participant = (coviu_sfu_participant *)session->participant;
	if((!video && participant->audio_active) || (video && participant->video_active)) {
		...
		/* Done, relay it */
		coviu_sfu_rtp_relay_packet packet;
		packet.data = rtp;
		packet.length = len;
		...
		/* Go */
		g_slist_foreach(participant->listeners, coviu_sfu_relay_rtp_packet, &packet);
...
void coviu_sfu_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	if(session->destroyed)
		return;
	if(session->participant_type == coviu_sfu_p_type_subscriber) {
...
void coviu_sfu_incoming_data(janus_plugin_session *handle, char *buf, int len) {
	...
	*(text+len) = '\0';
	...
	g_slist_foreach(participant->listeners, coviu_sfu_relay_data_packet, text);
	g_free(text);
}
...
void coviu_sfu_slow_link(janus_plugin_session *handle, int uplink, int video) {
	...
	if(session->participant_type == coviu_sfu_p_type_publisher) {
		if(!uplink) {
			coviu_sfu_participant *publisher = (coviu_sfu_participant *)session->participant;
			if(publisher) {
				...
				json_decref(event);
				event = NULL;
				gateway->push_event(session->handle, &coviu_sfu_plugin, NULL, event_text, NULL, NULL);
				g_free(event_text);
				event_text = NULL;
			}
		} else {
		...
		if(uplink) {
			coviu_sfu_listener *viewer = (coviu_sfu_listener *)session->participant;
			if(viewer) {
				/* Send an event on the handle to notify the application: it's
				 * up to the application to then choose a policy and enforce it */
				json_t *event = json_object();
...
static void coviu_sfu_recorder_create(coviu_sfu_participant *participant, gboolean audio, gboolean video) {
	char filename[255];
	...
	if(audio) {
		...
		if(participant->recording_base) {
			...
			participant->arc = janus_recorder_create(participant->room->rec_dir,
				coviu_sfu_audiocodec_name(participant->room->acodec), filename);
	...
	if(video) {
		...
		if(participant->recording_base) {
			...
			participant->vrc = janus_recorder_create(participant->room->rec_dir,
				coviu_sfu_videocodec_name(participant->room->vcodec), filename);
...
static void coviu_sfu_recorder_close(coviu_sfu_participant *participant) {
	if(participant->arc) {
...
void coviu_sfu_hangup_media(janus_plugin_session *handle) {
	...
	if(session->participant_type == coviu_sfu_p_type_publisher) {
		...
		coviu_sfu_recorder_close(participant);
		janus_mutex_unlock(&participant->rec_mutex);
		janus_mutex_lock(&participant->listeners_mutex);
		while(participant->listeners) {
			coviu_sfu_listener *l = (coviu_sfu_listener *)participant->listeners->data;
			if(l) {
		...
		coviu_sfu_leave_or_unpublish(participant, FALSE);
	} else if(session->participant_type == coviu_sfu_p_type_subscriber) {
		/* Get rid of listener */
		coviu_sfu_listener *listener = (coviu_sfu_listener *)session->participant;
		if(listener) {
			listener->paused = TRUE;
			coviu_sfu_participant *publisher = listener->feed;
			if(publisher != NULL) {
		...
		while(ps) {
			...
			if(l) {
				l->paused = TRUE;
				coviu_sfu_participant *publisher = l->feed;
				if(publisher != NULL) {
					janus_mutex_lock(&publisher->listeners_mutex);
...
static void coviu_sfu_sdp_a_format(char *mline, int mline_size, coviu_sfu_audiocodec acodec, int pt, const char *audio_mode) {
	switch(acodec) {
		case COVIU_SFU_OPUS:
...
static void coviu_sfu_sdp_v_format(char *mline, int mline_size, coviu_sfu_videocodec vcodec, int pt, int b, const char *video_mode) {
	switch(vcodec) {
		case COVIU_SFU_VP8:
...
static void *coviu_sfu_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(msg == &exit_message)
			break;
		if(msg->handle == NULL) {
			coviu_sfu_message_free(msg);
			continue;
		}
		coviu_sfu_session *session = NULL;
		janus_mutex_lock(&sessions_mutex);
		if(g_hash_table_lookup(sessions, msg->handle) != NULL ) {
			session = (coviu_sfu_session *)msg->handle->plugin_handle;
		}
		janus_mutex_unlock(&sessions_mutex);
		if(!session) {
			JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
		...
		if(session->participant_type == coviu_sfu_p_type_none) {
			...
			if(strcasecmp(request_text, "join") && strcasecmp(request_text, "joinandconfigure")) {
				...
				error_code = COVIU_SFU_ERROR_JOIN_FIRST;
			...
			janus_mutex_lock(&rooms_mutex);
			coviu_sfu *videoroom = NULL;
			error_code = coviu_sfu_access_room(root, FALSE, TRUE, &videoroom, error_cause, sizeof(error_cause));
			janus_mutex_unlock(&rooms_mutex);
			if(error_code != 0)
				goto error;
			...
			if(!strcasecmp(ptype_text, "publisher")) {
				...
				if(id) {
					...
					if(g_hash_table_lookup(videoroom->participants, &user_id) != NULL) {
						...
						error_code = COVIU_SFU_ERROR_ID_EXISTS;
				...
				coviu_sfu_participant *publisher = g_malloc0(sizeof(coviu_sfu_participant));
				if(publisher == NULL) {
					JANUS_LOG(LOG_FATAL, "Memory error!\n");
					error_code = COVIU_SFU_ERROR_UNKNOWN_ERROR;
					g_snprintf(error_cause, 512, "Memory error");
				...
				/* Done */
				session->participant_type = coviu_sfu_p_type_publisher;
				...
				janus_mutex_lock(&videoroom->participants_mutex);
				coviu_sfu_participant *publisher = g_hash_table_lookup(videoroom->participants, &feed_id);
				janus_mutex_unlock(&videoroom->participants_mutex);
				if(publisher == NULL || publisher->sdp == NULL) {
					...
					coviu_sfu_listener *listener = g_malloc0(sizeof(coviu_sfu_listener));
					if(listener == NULL) {
						JANUS_LOG(LOG_FATAL, "Memory error!\n");
						error_code = COVIU_SFU_ERROR_UNKNOWN_ERROR;
						g_snprintf(error_cause, 512, "Memory error");
					...
					session->participant_type = coviu_sfu_p_type_subscriber;
					...
					if(publisher->sdp != NULL) {
						...
						int res = gateway->push_event(msg->handle, &coviu_sfu_plugin, msg->transaction, event_text, "offer", publisher->sdp);
				...
				/* Allocate listener */
				coviu_sfu_listener_muxed *listener = g_malloc0(sizeof(coviu_sfu_listener_muxed));
				if(listener == NULL) {
					JANUS_LOG(LOG_FATAL, "Memory error!\n");
					error_code = COVIU_SFU_ERROR_UNKNOWN_ERROR;
					g_snprintf(error_cause, 512, "Memory error");
				...
				listener->session = session;
				listener->room = videoroom;
				session->participant_type = coviu_sfu_p_type_subscriber_muxed;
				session->participant = listener;
				...
				/* How long will the gateway take to push the event? */
				gint64 start = janus_get_monotonic_time();
				int res = gateway->push_event(msg->handle, &coviu_sfu_plugin, msg->transaction, event_text, NULL, NULL);
				JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (took %"SCNu64" us)\n", res, janus_get_monotonic_time()-start);
				JANUS_LOG(LOG_VERB, "  >> %d\n", res);
				g_free(event_text);
				root = NULL;
				...
				if(list != NULL) {
					...
					if(videoroom->destroyed || coviu_sfu_muxed_subscribe(listener, list, msg->transaction) < 0) {
						JANUS_LOG(LOG_ERR, "Error subscribing!\n");
						error_code = COVIU_SFU_ERROR_UNKNOWN_ERROR;	/* FIXME */
			...
			if(!strcasecmp(request_text, "join") || !strcasecmp(request_text, "joinandconfigure")) {
				...
				error_code = COVIU_SFU_ERROR_ALREADY_JOINED;
				...
				if(!strcasecmp(request_text, "publish") && participant->sdp) {
					...
					error_code = COVIU_SFU_ERROR_ALREADY_PUBLISHED;
				...
				if(participant->recording_active != prev_recording_active) {
					...
					if(!participant->recording_active) {
						...
						coviu_sfu_recorder_create(participant, strstr(participant->sdp, "m=audio") != NULL, strstr(participant->sdp, "m=video") != NULL);
				...
				if(!participant->sdp) {
					...
					error_code = COVIU_SFU_ERROR_NOT_PUBLISHED;
			...
			if(!strcasecmp(request_text, "join")) {
				...
				coviu_sfu_participant *publisher = g_hash_table_lookup(listener->room->participants, &feed_id);
				...
				coviu_sfu_participant *prev_feed = listener->feed;
				if(prev_feed) {
			...
			if(!strcasecmp(request_text, "join")) {
				...
				list = g_list_reverse(list);
				if(coviu_sfu_muxed_subscribe(listener, list, msg->transaction) < 0) {
					JANUS_LOG(LOG_ERR, "Error subscribing!\n");
					error_code = COVIU_SFU_ERROR_UNKNOWN_ERROR;	/* FIXME */
					g_snprintf(error_cause, 512, "Error subscribing!");
					goto error;
				...
				if(coviu_sfu_muxed_unsubscribe(listener, list, msg->transaction) < 0) {
					JANUS_LOG(LOG_ERR, "Error unsubscribing!\n");
					error_code = COVIU_SFU_ERROR_UNKNOWN_ERROR;	/* FIXME */
					g_snprintf(error_cause, 512, "Error unsubscribing!");
					goto error;
		...
		/* Any SDP to handle? */
		if(!msg->sdp) {
			int ret = gateway->push_event(msg->handle, &coviu_sfu_plugin, msg->transaction, event_text, NULL, NULL);
			JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
		} else {
			...
			if(!strcasecmp(msg->sdp_type, "offer")) {
				...
				error_code = COVIU_SFU_ERROR_INVALID_SDP_TYPE;
			...
			if(session->participant_type != coviu_sfu_p_type_publisher) {
				...
				if(count == videoroom->max_publishers) {
					...
					error_code = COVIU_SFU_ERROR_PUBLISHERS_FULL;
				...
				for(pass = 1; pass <= 2; pass++) {
					...
					if(audio) {
						...
						if(pt >= 0)
							coviu_sfu_sdp_a_format(audio_mline, 256, videoroom->acodec, pt, audio_mode);
					...
					if(video) {
						...
						if(pt >= 0)
							coviu_sfu_sdp_v_format(video_mline, 512, videoroom->vcodec, pt, b, video_mode);
					...
					if(videoroom->record || participant->recording_active) {
						coviu_sfu_recorder_create(participant, audio, video);
					}
					janus_mutex_unlock(&participant->rec_mutex);
					...
					g_atomic_int_set(&session->hangingup, 0);
					gint64 start = janus_get_monotonic_time();
					res = gateway->push_event(msg->handle, &coviu_sfu_plugin, msg->transaction, event_text, type, newsdp);
					JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (took %"SCNu64" us)\n", res, janus_get_monotonic_time()-start);
					g_free(newsdp);
				}
				/* Done */
...
int coviu_sfu_muxed_subscribe(coviu_sfu_listener_muxed *muxed_listener, GList *feeds, char *transaction) {
	...
	coviu_sfu *videoroom = muxed_listener->room;
	GList *ps = feeds;
	json_t *list = json_array();
	int added_feeds = 0;
	while(ps) {
		...
		while(ls) {
			coviu_sfu_listener *l = (coviu_sfu_listener *)ls->data;
	...
	return coviu_sfu_muxed_offer(muxed_listener, transaction, event_text);
}
...
int coviu_sfu_muxed_unsubscribe(coviu_sfu_listener_muxed *muxed_listener, GList *feeds, char *transaction) {
	...
	while(ps) {
		...
		GSList *ls = muxed_listener->listeners;
		while(ls) {
			coviu_sfu_listener *listener = (coviu_sfu_listener *)ls->data;
			if(listener) {
				coviu_sfu_participant *publisher = listener->feed;
				...
				JANUS_LOG(LOG_VERB, "Now subscribed to %d feeds\n", g_slist_length(muxed_listener->listeners));
				coviu_sfu_listener_free(listener);
				/* Add to feeds in the answer */
				removed_feeds++;
				json_t *f = json_object();
				json_object_set_new(f, "id", json_integer(feed_id));
...
int coviu_sfu_muxed_offer(coviu_sfu_listener_muxed *muxed_listener, char *transaction, char *event_text) {
	...
	coviu_sfu_sdp_a_format(audio_mline, 512, muxed_listener->room->acodec, pt, "sendonly");
	pt = -1;
	...
	coviu_sfu_sdp_v_format(video_mline, 512, muxed_listener->room->vcodec, pt, 0, "sendonly");
	...
	/* How long will the gateway take to push the event? */
	gint64 start = janus_get_monotonic_time();
	int res = gateway->push_event(muxed_listener->session->handle, &coviu_sfu_plugin, transaction, event_text, "offer", newsdp);
	JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (took %"SCNu64" us)\n", res, janus_get_monotonic_time()-start);
	if(res != JANUS_OK) {
...
static void coviu_sfu_relay_rtp_packet(gpointer data, gpointer user_data) {
	coviu_sfu_rtp_relay_packet *packet = (coviu_sfu_rtp_relay_packet *)user_data;
	if(!packet || !packet->data || packet->length < 1) {
		JANUS_LOG(LOG_ERR, "Invalid packet...\n");
		return;
	}
	coviu_sfu_listener *listener = (coviu_sfu_listener *)data;
	...
	coviu_sfu_session *session = listener->session;
...
/* Helper to free coviu_sfu structs. */
static void coviu_sfu_free(coviu_sfu *room) {
	if(room) {
...
static void coviu_sfu_participant_free(coviu_sfu_participant *p) {
	...
	while(p->listeners) {
		coviu_sfu_listener *l = (coviu_sfu_listener *)p->listeners->data;
		if(l) {
